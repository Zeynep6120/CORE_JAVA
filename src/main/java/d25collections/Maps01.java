...
public class Maps01 {
    public static void main(String[] args) {
        ...
        myMap.put("Ali Can", 25);//key tekrarlı kullanıldığında son value aktif olur. Üstüne yazar.
        ...
        System.out.println(valueSide);//[25, 15, 18, 19] //collectionun sırası da rastgele yani insert sırası değil

        //get(): Bir key'e karşılık gelen değeri döndürür.key verirsiniz value’sunu getirir
        Integer age = myMap.get("Ali Can");// bu method büyük küçük harf duyarlıdır  o yüzden tam aynı key i yazmalıyız.
        System.out.println(age); //13  key verdik value yu getirdi
        ...
        for (Integer w : ages) { // biz burda map in value tarafını almayı öğrendik
            total = total + w;
        }
        ...
        //Map Set e döndü çünkü artık süslü parantez değişti ve köşeli parantez oldu.artık bu yapı set yapısı oldu
        //artık biz set i döngüye sokabiliriz çünkü
        //burda map i döngüye sokkmaya çalışıyoruz  Ali Can=25  bu iki artık bir veri olarak görülüyor .iki ayrı veri olarka görülmüyor
        //Set<Map.Entry<String, Integer>> //Map a gittim Entry methodunu çağırdım.Entry key ve value yi aynı anda yazmamı sağlıyor
        //normalde set in içine hem string hem de Integer değer yazamam.
        //Map.Entry<String, Integer> burası kendi başına bir data tipidir.bura string,string olabilir string,integer  olabilir
        //Set<> bunun içine  iki tane data tipi yazamadığım için yukardaki map.entry data tipinden yardım aldım
        //Biz bu kadar uğrasi yani yukardaki data tiplerini kullanmamızın sebebi map i döngüye sokmak için for kullanmak için
        //çünkü bir döngünün içine  direkt map i sokamayız

        for (Map.Entry<String, Integer> w : myEntrySet) {//[Ali Can=25, Ayşe Kaya=15, Veli Han=18, Fatma Yılmaz=19]
            //Map.Entry<String,Integer>  bu string,İnteger gibi bir data tipidir
        ...
        // for ile map i döngüye sokmuş gibi her bir veriye tek tek ulaştık
